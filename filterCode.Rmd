...
state_pop2018 <- read.csv("nst-est2018-alldata.csv")[,c('NAME', 'POPESTIMATE2018')]
state_pop2018$state <- tolower(state_pop2018$NAME)

us_data <- map_data("state")

incidents_by_state <- gun_violence2018 %>% 
  group_by(state) %>% 
  summarise(count = n())

standard_by_state <- incidents_by_state %>%
  left_join(state_pop2018, by = "state")

standard_by_state$gv_per_100thou <- (standard_by_state$count/standard_by_state$POPESTIMATE2018)*100000

us_data <- us_data %>%
  left_join(standard_by_state, by = c("region" = "state"))

i = 1
for(i in 1:nrow(us_data)){
  if(us_data$gv_per_100thou[i] <= 3.317){
    us_data$gv_cat[i] = "1"
  } else if(3.317 < us_data$gv_per_100thou[i] & us_data$gv_per_100thou[i] <= 4.789){
    us_data$gv_cat[i] = "2"
  } else if(4.789 < us_data$gv_per_100thou[i] & us_data$gv_per_100thou[i] <= 6.140){
    us_data$gv_cat[i] = "3"
  } else{
    us_data$gv_cat[i] = "4"
  }
...
new_us_data <- select(us_data,long,lat,group,gv_cat)
write.csv(new_us_data,"us_data.csv",row.names = FALSE)
