...
deaths <- mutate(deaths, date = as.Date(date, format = "%Y-%m-%d")) %>%
...
penn_deaths <- select(n_killed,longitude,latitude,date,n_injured,source_url)
write.csv(penn_deaths,"pennDeadly.csv",row.names = FALSE)

write.csv(age_data_region,"pennDeadly.csv",row.names = FALSE)
```
